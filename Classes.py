...
class Database:
    ...
    # TODO: remove_users_upload
    def remove_users_upload(id):  # remove_nhentai
        result, data = DATABASE.select_one_row(f"SELECT ID, Name FROM nhentai WHERE ID == {id};")
        ...
        result, data = DATABASE.remove(f"DELETE FROM nhentai WHERE ID == {ID};")
        if result is False:
            return -1, data
